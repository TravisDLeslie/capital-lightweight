...
function ChatProductOptions({
  ...
  const isDiabloTools = products.every(
    (product) => product.category === 'Diablo Tools',
  )
  const visibleLimit = isDiabloTools ? 10 : 4
  const visibleProducts = showAll ? products : products.slice(0, visibleLimit)
  ...
  const helperText = isFenceEstimate
    ...
    : isDiabloTools
    ? 'Tap a row to see model number, UPC, price, and product details.'
  ...
  return (
    <div className="space-y-4">
      ...
      {isDiabloTools && products.length > 6 ? (
        <div className="overflow-hidden rounded-lg border border-stone-200 bg-white">
          <div className="hidden grid-cols-[7rem_minmax(0,1fr)_8.5rem_5.5rem] gap-3 border-b border-stone-200 bg-stone-50 px-3 py-2 text-[11px] font-black uppercase tracking-wide text-stone-500 md:grid">
            <span>Model #</span>
          ...
          <div className="max-h-[28rem] overflow-y-auto">
            {visibleProducts.map((product) => {
              ...
              const description = product.name.replace(/^Diablo\s+/i, '')

              return (
                <button
                  className="block w-full border-b border-stone-100 px-3 py-3 text-left transition last:border-b-0 hover:bg-red-50/60 focus:bg-red-50 focus:outline-none md:grid md:grid-cols-[7rem_minmax(0,1fr)_8.5rem_5.5rem] md:items-center md:gap-3"
                  key={product.id}
                ...
                >
                  <div className="flex items-start gap-3 md:contents">
                    <img
                      alt=""
                      className="h-12 w-12 shrink-0 rounded bg-red-50 object-contain p-1 ring-1 ring-red-100 md:hidden"
                      src={product.image}
                    />
                    <div className="min-w-0 flex-1 md:contents">
                      <div className="font-black text-stone-950 md:text-sm">
                        {product.modelNumber || product.stockSku || product.id}
                      </div>
                      <div className="mt-1 min-w-0 text-sm font-semibold leading-5 text-stone-800 md:mt-0">
                        {description}
                        <span className="mt-1 block text-xs font-bold text-emerald-700 md:hidden">
                          {availability.label}
                        </span>
                      </div>
                      <div className="mt-1 text-xs font-semibold text-stone-500 md:mt-0 md:text-sm">
                        {product.upc}
                      </div>
                      <div className="mt-2 flex items-center justify-between gap-3 md:mt-0 md:block md:text-right">
                        <span className="inline-flex rounded-full bg-emerald-50 px-2 py-1 text-xs font-bold text-emerald-700 md:hidden">
                          {availability.label}
                        </span>
